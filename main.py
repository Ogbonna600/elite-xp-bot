...
async def linktwitter_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    create_user(user.id, user.username, user.first_name)
...
async def daily_leaderboard_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def verify_engagement(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    user_data = get_user(user.id)
...
# Scheduled functions
async def post_train_schedule(context: ContextTypes.DEFAULT_TYPE):
    job = context.job
    train_info = job.data
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "join_daily":
        ...
        await joindaily_cmd(mock_update, mock_context)
...
def run_bot():
    ...
    application.add_handler(CommandHandler("linktwitter", linktwitter_cmd))
    application.add_handler(CommandHandler("myxp", myxp))
    application.add_handler(CommandHandler("leaderboard", leaderboard))
    application.add_handler(CommandHandler("dailyleaderboard", daily_leaderboard_cmd))
    application.add_handler(CommandHandler("joindaily", joindaily_cmd))
    application.add_handler(CommandHandler("verify", verify_engagement))
